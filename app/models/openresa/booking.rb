class Openresa::Booking
  ...
  @@available_fields = %w( id name href prod_id checkin checkout partner_id partner_order_id partner_type contact_phone partner_mail people_name people_email people_phone is_citizen create_date write_date deleted_at state state_num actions reservation_line create_uid write_uid all_dispo recurrence_id is_template partner_invoice_id note confirm_note cancel_note done_note pricelist_id state_event resource_ids resources people_street people_city people_zip whole_day invoice_attachment_id)
